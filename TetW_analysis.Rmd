...
```{r}
TetW_fastq_files <- "/Volumes/AESexternal/AMR/TetW_5_20_22/TetW_redo_AES-350564332/TetW_fastq" #list directory where fastq files are 
...
saveRDS(TetW_out, "~/Desktop/grad_school/AMR_project/TetW_analysis/TetW_out.RDS")
...
write.table(TetW_asv_fasta, "/Users/arenasee/Desktop/grad_school/AMR_project/TetW_analysis/ASVs_TetW.fa", row.names=FALSE,sep="\t", quote = FALSE)
...
write.table(TetW_asv_tab, "/Users/arenasee/Desktop/grad_school/AMR_project/TetW_analysis/ASVs_counts_TetW.fa", sep="\t", quote=F)
...
```{r}
pdf("/Users/arenasee/Desktop/grad_school/Thesis/Figures/TetW/ASV_abund_HM.pdf")
...
```{r}
plot_heatmap(tetw_ps_rumen, sample.label="DayOfStudy", sample.order="DayOfStudy")
```
...
#DESEQ heatmap -- comparison between treatments
```{r}
#phyloseq object that is not normalised!
tetw_ps_all
##create an object that will tell deseq what to compare, in this case Breed, you will want to change this to Treatment, then Timepoint
...
#finding top 10 sig diff ASVs
tetw_top20.tr <- head(tetw_treat.st, 20)
```
...
################################## MERGING ASV TABLES FOR NET ANALYSIS ################################
```{r}
#original phyloseq objects had illumina indexes as rownames - had to be reordered according to tube no (which must match exactly) 
R16s_ps_reord <- set_sample_order(R16s_inj_phylobj, "Tube")
tetw_rum_reord <- set_sample_order(tetw_ps_rumen, "Tube")

## merging ASV tables
merged_otu <- merge_phyloseq_pair(otu_table(tetw_rum_reord), otu_table(R16s_ps_reord))
write.table(merged_otu, file="merged_otu.txt", col.names=NA, row.names=T, sep = "\t",quote=F)
```   

#fecal
```{r}
#F16s_inj_phylobj <- readRDS("F16s_inj_phylobj.RDS")

#adding "T_" to TetW ASVs so they can be differentiated from 16s ASVs
taxa_names(tetw_ps_fecal) <- paste("T_", taxa_names(tetw_ps_fecal), sep="")
taxa_names(tetw_ps_fecal)

#setting sample # according to tube no
F16s_ps_reord <- set_sample_order(F16s_inj_phylobj, "Tube")
tetw_fec_reord <- set_sample_order(tetw_ps_fecal, "Tube")

## merging ASV tables
F16s_tet.asv <- merge_phyloseq_pair(otu_table(F16s_ps_reord), otu_table(tetw_fec_reord))
write.table(F16s_tet.asv, file="F16s_tet.asv.txt", col.names=NA, row.names=T, sep = "\t",quote=F)
```
