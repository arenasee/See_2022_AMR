...
```{r}
ermB_fastq_files <- "/Volumes/Arena/AMR/ermB_2_24_22/ermB_redo_aes-335090892/ermB_fastq" #where  fastq files are
...
###DESEQ rumen contrast between AbxFree + Nuflor

```{r}
#run the algorithm
#make a results table of the algorithm
ermB_rumres_AN = DESeq2::results(ermB_rum_dds, contrast=c("Treatment", "AbxFree", "Nuflor")) 
#shows summary of results
summary(ermB_rumres_AN) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_rum_stAN = ermB_rumres_AN[which(ermB_rumres_AN$padj < alpha), ] 
View(ermB_rum_stAN)

#printing row names
rnames_AN <- rownames(ermB_rum_stAN)


ermB_rum_stAN <- data.frame(ermB_rum_stAN)
write.table(ermB_rum_stAN,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/ermB_rum_AN_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
ER_AN <- ggplot(ermB_rum_stAN, aes(x=rnames_AN, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Rumen - Differential ASVs between AbxFree vs Nuflor treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/ER_AN.pdf", width = 30, height = 18, units="cm")
```

#DESEQ rumen contrast between AbxFree + Draxxin

```{r}
#run the algorithm
#make a results table of the algorithm
ermB_rumres_AD = DESeq2::results(ermB_rum_dds, contrast=c("Treatment", "AbxFree", "Draxxin")) 
#shows summary of results
View(ermB_rumres_AD) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_rum_stAD = ermB_rumres_AD[which(ermB_rumres_AD$padj < alpha),] 
View(ermB_rum_stAD)

#printing row names
rnames_AD <- rownames(ermB_rum_stAD)
row.names(ermB_rum_stAD) <- rnames_AD

ermB_rum_stAD <- data.frame(ermB_rum_stAD)
write.table(ermB_rum_stAD,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/ermB_rum_AD_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
ER_AD <- ggplot(ermB_rum_stAD, aes(x=rnames_AD, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Rumen - Differential ASVs between AbxFree vs Draxxin treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/ER_AD.pdf", width = 30, height = 18, units="cm")
```

#DESEQ rumen contrast between AbxFree + Excede
```{r}
#run the algorithm
#make a results table of the algorithm
ermB_rumres_AE = DESeq2::results(ermB_rum_dds, contrast=c("Treatment", "AbxFree", "Excede")) 
#shows summary of results
View(ermB_rumres_AE) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_rum_stAE = ermB_rumres_AE[which(ermB_rumres_AE$padj < alpha), ] 

#printing row names
rnames_AE <- rownames(ermB_rum_stAE)
row.names(ermB_rum_stAE) <- rnames_AE

#forming a plot of the differential ASVs
ermB_rum_stAE <- data.frame(ermB_rum_stAE)
write.table(ermB_rum_stAE,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/ermB_rum_AE_sigtab.txt", sep="\t", quote=F)

ER_AE  <- ggplot(ermB_rum_stAE, aes(x=rnames_AE, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Rumen - Differential ASVs between AbxFree vs Excede treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/ER_AE.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_rum_stAE, "ermB_rum_stAE.RDS")
saveRDS(ermB_rumres_AE, "ermB_rumres_AE.RDS")
```

#DESEQ rumen contrast AbxFree + Baytril

```{r}
#make a results table of the algorithm
ermB_rumres_AB = DESeq2::results(ermB_rum_dds, contrast=c("Treatment", "AbxFree", "Baytril")) #
#shows summary of results
View(ermB_rumres_AB) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_rum_stAB = ermB_rumres_AB[which(ermB_rumres_AB$padj < alpha), ] 
View(ermB_rum_stAB)

#printing row names
rnames_AB <- rownames(ermB_rum_stAB)
row.names(ermB_rum_stAB) <- rnames_AB

ermB_rum_stAB <- data.frame(ermB_rum_stAB)
write.table(ermB_rum_stAB,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/ermB_rum_AB_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
ER_AB <- ggplot(ermB_rum_stAB, aes(x=rnames_AB, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Rumen - Differential ASVs between AbxFree vs Baytril treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/ER_AB.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_rumres_AB, "ermB_rumres_AB.RDS")
saveRDS(ermB_rum_stAB, "ermB_rum_stAB.RDS")
```

#forming upset diagram of common rumen ASVs between treatments
```{r}
library(UpSetR)
ermB_rum_common_ASV <- as.vector(list(
  Nuflor = rnames_AN, 
  Draxxin = rnames_AD, 
  Excede = rnames_AE, 
  Baytril = rnames_AB))

pdf("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/rumen_upset.pdf")
ermB_rum_upset <- upset(fromList(ermB_rum_common_ASV), 
                            order.by = "freq", point.size = 3, line.size = 1,
                            mainbar.y.label = "Differential ASV Intersections", 
                            sets.x.label = "Antibiotic Treatment", 
                            text.scale = c(1.3, 1.3, 1, 1, 2, 0.75),
                            sets.bar.color=c("#ECCBAE", "#046C9A", "#D69C4E", "#ABDDDE"),
                            matrix.color = "black",
...
#FECAL - DESEQ contrast AbxFree vs Baytril
```{r}
#make a results table of the algorithm
ermB_fecres_AB = DESeq2::results(ermB_fec_dds, contrast=c("Treatment", "AbxFree", "Baytril")) #
#shows summary of results
View(ermB_fecres_AB) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_fec_stAB = ermB_fecres_AB[which(ermB_fecres_AB$padj < alpha), ] 
View(ermB_fec_stAB)

#printing row names
rnames_AB <- rownames(ermB_fec_stAB)
row.names(ermB_fec_stAB) <- rnames_AB

ermB_fec_stAB <- data.frame(ermB_fec_stAB)
write.table(ermB_fec_stAB,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/fecal_AB_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
EF_AB <- ggplot(ermB_fec_stAB, aes(x=rnames_AB, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Fecal - Differential ASVs between AbxFree vs Baytril treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/Fecal_AB.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_fecres_AB, "ermB_fecres_AB.RDS")
saveRDS(ermB_fec_stAB, "ermB_fec_stAB.RDS")
```

# FECAL - DESEQ contrast AbxFree vs Draxxin

```{r}
#make a results table of the algorithm
ermB_fecres_AD = DESeq2::results(ermB_fec_dds, contrast=c("Treatment", "AbxFree", "Draxxin")) 
#shows summary of results
View(ermB_fecres_AD) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_fec_stAD = ermB_fecres_AD[which(ermB_fecres_AD$padj < alpha), ] 

#printing row names
rnames_AD <- rownames(ermB_fec_stAD)
row.names(ermB_fec_stAD) <- rnames_AD

ermB_fec_stAD <- data.frame(ermB_fec_stAD)
write.table(ermB_fec_stAD,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/Fecal_AD_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
EF_AD <- ggplot(ermB_fec_stAD, aes(x=rnames_AD, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Fecal - Differential ASVs between AbxFree vs Draxxin treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/Fecal_AD.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_fecres_AD, "ermB_fecres_AD.RDS")
saveRDS(ermB_fec_stAD, "ermB_fec_stAD.RDS")
```

#FECAL - DESEQ contrast AbxFree vs Excede

```{r}
#make a results table of the algorithm
ermB_fecres_AE = DESeq2::results(ermB_fec_dds, contrast=c("Treatment", "AbxFree", "Excede")) 
#shows summary of results
View(ermB_fecres_AE) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_fec_stAE = ermB_fecres_AE[which(ermB_fecres_AE$padj < alpha), ] 

#printing row names
rnames_AE <- rownames(ermB_fec_stAE)
row.names(ermB_fec_stAE) <- rnames_AE

ermB_fec_stAE <- data.frame(ermB_fec_stAE)
write.table(ermB_fec_stAE,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/Fecal_AE_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
EF_AE <- ggplot(ermB_fec_stAE, aes(x=rnames_AE, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Fecal - Differential ASVs between AbxFree vs Excede treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/Fecal_AE.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_fecres_AE, "ermB_fecres_AE.RDS")
saveRDS(ermB_fec_stAE, "ermB_fec_stAE.RDS")
```

#FECAL - DESEQ contrast AbxFree vs Nuflor

```{r}
#make a results table of the algorithm
ermB_fecres_AN = DESeq2::results(ermB_fec_dds, contrast=c("Treatment", "AbxFree", "Nuflor")) 
#shows summary of results
View(ermB_fecres_AN) 
#create a metric to filter your results
alpha = 0.05 
#filter out asvs from the result table that do not meet alpha
ermB_fec_stAN = ermB_fecres_AN[which(ermB_fecres_AN$padj < alpha), ] 

#printing row names
rnames_AN <- rownames(ermB_fec_stAN)
row.names(ermB_fec_stAN) <- rnames_AN

ermB_fec_stAN <- data.frame(ermB_fec_stAN)
write.table(ermB_fec_stAN,"/Users/arenasee/Desktop/grad_school/AMR_project/ermB_analysis/Fecal_AN_sigtab.txt", sep="\t", quote=F)

#forming a plot of the differential ASVs
EF_AN <- ggplot(ermB_fec_stAN, aes(x=rnames_AN, y=log2FoldChange, color=padj)) + geom_point(size=6) + scale_colour_gradient(low = "#446455", high = "#FDD262") + theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5), axis.text = element_text(size = 10), axis.title=element_text(size=10), legend.position="right", legend.direction = "vertical", legend.text=element_text(size=10)) + ggtitle("ermB Fecal - Differential ASVs between AbxFree vs Nuflor treatments") 
ggsave("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/Fecal_AN.pdf", width = 30, height = 18, units="cm")

saveRDS(ermB_fecres_AN, "ermB_fecres_AN.RDS")
saveRDS(ermB_fec_stAN, "ermB_fec_stAN.RDS")
```

# UPSET plot
```{r}
ermB_fec_common_ASV <- as.vector(list(
  Nuflor = rnames_AN, 
  Draxxin = rnames_AD, 
  Excede = rnames_AE, 
  Baytril = rnames_AB))

pdf("/Users/arenasee/Desktop/grad_school/Thesis/Figures/ermB/fecal_upset.pdf")
ermB_fec_upset <- upset(fromList(ermB_fec_common_ASV), 
                            order.by = "freq", point.size = 3, line.size = 1,
                            mainbar.y.label = "Differential ASV Intersections", 
                            sets.x.label = "Antibiotic Treatment", 
                            text.scale = c(1.3, 1.3, 1, 1, 2, 0.75),
